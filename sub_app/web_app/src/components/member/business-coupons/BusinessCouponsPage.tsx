...
import CouponsTable from "../../admin/coupons/CouponsTable";
import CouponFormModal, { couponToForm, emptyForm, formToPayload, parseProductIDs } from "../../admin/coupons/CouponFormModal";
import type { FormState } from "../../admin/coupons/CouponFormModal";
import type { Coupon } from "../../admin/coupons/types";
import { createMemberBusinessCoupon, deleteMemberBusinessCoupon, listMemberBusinessCoupons, updateMemberBusinessCoupon } from "./api";
...
export default function BusinessCouponsPage() {
	...
	const loadData = useCallback(async () => {
		...
		setError(null);
		try {
			const res = await listMemberBusinessCoupons(selectedBusinessId, { q, is_active: activeFilter, page, limit });
			setCoupons(res.data || []);
	...
	const handleFormChange = (patch: Partial<FormState>) => {
		setFormState((prev) => {
			...
			if (patch.product_ids_text !== undefined) {
	...
	const handleRemoveSelectedProduct = (id: string) => {
		syncSelectedProducts(formState.selectedProductIDs.filter((value) => value !== id));
	};
	...
	const handleFormSubmit = async () => {
		...
		try {
			...
			if (formMode === "create") {
				await createMemberBusinessCoupon(selectedBusinessId, payload);
				notifySuccess("Coupon created");
			} else if (selectedCoupon) {
				await updateMemberBusinessCoupon(selectedBusinessId, selectedCoupon.id, payload);
	...
	const handleConfirmDelete = async () => {
		...
		try {
			await deleteMemberBusinessCoupon(selectedBusinessId, selectedCoupon.id);
		...
		<div className="space-y-4">
			<div className="flex flex-wrap items-start justify-between gap-3">
				<div>
					...
					<p className="text-sm text-slate-600">Kelola voucher yang terikat ke business tertentu.</p>
				</div>
				<button type="button" onClick={handleCreate} className="rounded-lg bg-slate-900 px-3 py-2 text-sm font-medium text-white hover:bg-slate-800" disabled={!selectedBusinessId}>
					+ New Business Coupon
				</button>
			</div>
			...
			<div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
				<div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
					...
					<div className="min-w-[280px]">
						<select className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm" value={selectedBusinessId} onChange={(event) => { setPage(1); setSelectedBusinessId(event.target.value); }}>
							<option value="">Pilih business</option>
							{businesses.map((business) => (
								<option key={business.id} value={business.id}>{business.name} / {business.slug}</option>
							))}
						</select>
					</div>
				</div>
			...
			<div className="grid gap-2 rounded-xl border border-slate-200 bg-white p-3 sm:grid-cols-3">
				<input className="rounded border border-slate-300 px-2 py-1.5 text-sm" placeholder="Search code / name" value={q} onChange={(e) => { setPage(1); setQ(e.target.value); }} />
				<select className="rounded border border-slate-300 px-2 py-1.5 text-sm" value={activeFilter} onChange={(e) => { setPage(1); setActiveFilter(e.target.value as "" | "true" | "false"); }}>
					<option value="">All status</option>
					<option value="true">Active only</option>
					<option value="false">Inactive only</option>
				</select>
				<select className="rounded border border-slate-300 px-2 py-1.5 text-sm" value={limit} onChange={(e) => { setLimit(Number(e.target.value)); setPage(1); }}>
					{[10, 20, 50].map((value) => (
			...
			{totalPages > 1 && (
				<div className="flex items-center justify-between text-sm text-slate-600">
					...
					<div className="flex gap-2">
						<button type="button" disabled={page <= 1} onClick={() => setPage((current) => current - 1)} className="rounded bg-slate-100 px-3 py-1 hover:bg-slate-200 disabled:opacity-40">Prev</button>
						<button type="button" disabled={page >= totalPages} onClick={() => setPage((current) => current + 1)} className="rounded bg-slate-100 px-3 py-1 hover:bg-slate-200 disabled:opacity-40">Next</button>
					</div>
				</div>
			)}
			...
			<EntityDeleteModal
				...
				description={`Hapus coupon "${selectedCoupon?.name}"?`}
