...
type Props = {
  onFilesAdded: (files: MemberUploadFile[]) => void;
  accept?: string;
  maxFiles?: number;
  maxSizeMB?: number;
};

export default function MemberFileUploadDropzone({ onFilesAdded, accept = "image/*,video/*", maxFiles = 10, maxSizeMB = 10 }: Props) {
  ...
  const processFiles = useCallback(
    (fileList: FileList | File[]) => {
      ...
      const validFiles: MemberUploadFile[] = [];
      const errors: string[] = [];
      ...
      files.forEach((file) => {
        ...
        const uploadFile: MemberUploadFile = {
          file,
          id,
          isMain: false,
          displayOrder: 0,
