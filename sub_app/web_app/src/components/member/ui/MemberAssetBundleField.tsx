...
export type MemberAssetRulesConfig = {
  type: string;
  rules: Record<string, MemberAssetRule>;
};

export type ExistingMemberAsset = {
  id: string;
  file_path: string;
  file_type: string;
  file_size?: number;
...
export type ExistingMemberAssetFolder = {
  id: string;
  business_id: string;
...
type Props = {
  title?: string;
  usageConfig: MemberAssetRulesConfig;
  usageTag: string;
  onUsageTagChange: (value: string) => void;
  enforceCrop?: boolean;
  selectedFiles: MemberUploadFile[];
  onSelectedFilesChange: (files: MemberUploadFile[]) => void;
  existingAssets?: ExistingMemberAsset[];
  loadingExisting?: boolean;
  showExisting?: boolean;
  folders?: ExistingMemberAssetFolder[];
  selectedFolderID?: string;
  onSelectedFolderIDChange?: (value: string) => void;
  onCreateFolder?: (name: string, parentID?: string) => Promise<void>;
  onRenameFolder?: (folder: ExistingMemberAssetFolder, nextName: string) => Promise<void>;
  onDeleteFolder?: (folder: ExistingMemberAssetFolder) => Promise<void>;
  maxFiles?: number;
  maxSizeMB?: number;
  accept?: string;
  onSetMainExisting?: (asset: ExistingMemberAsset) => Promise<void>;
  onDeleteExisting?: (asset: ExistingMemberAsset) => Promise<void>;
  onUpdateExisting?: (asset: ExistingMemberAsset, patch: { usage_tag?: string; display_order?: number; folder_id?: string | null }) => Promise<void>;
  onCopyExistingLink?: (asset: ExistingMemberAsset) => Promise<void>;
  onDuplicateExisting?: (asset: ExistingMemberAsset) => Promise<void>;
};
...
export async function validateMemberFilesAgainstUsageConfig(
  files: MemberUploadFile[],
  usageTag: string,
  usageConfig: MemberAssetRulesConfig
): Promise<string | null> {
  if (!usageTag) {
...
export default function MemberAssetBundleField({
  title = "Assets",
  usageConfig,
  usageTag,
  onUsageTagChange,
  ...
  type CropContext = { file: MemberUploadFile; mode: CropMode };
  ...
  const appendSelectedFiles = (files: MemberUploadFile[]) => {
    if (files.length === 0) {
      return;
    }
  ...
  const isCropRequired = (uploadFile: MemberUploadFile) => {
  ...
  const openCropper = (uploadFile: MemberUploadFile, mode: CropMode) => {
    revokeCropImageURL();
    setCropError("");
    setCrop({ x: 0, y: 0 });
    setZoom(1);
  ...
  const queueFilesForStrictCrop = (files: MemberUploadFile[]) => {
    pendingCropQueueRef.current = [...pendingCropQueueRef.current, ...files];
    drainCropQueue();
  };

  const handleFilesAdded = (files: MemberUploadFile[]) => {
    if (files.length === 0) {
      return;
    }
  ...
  const groupedExistingAssets = filteredExistingAssets.reduce<Record<string, ExistingMemberAsset[]>>((acc, asset) => {
  ...
  const openManualCropper = (uploadFile: MemberUploadFile) => {
    if (!uploadFile.file.type.startsWith("image/")) {
      return;
    }
