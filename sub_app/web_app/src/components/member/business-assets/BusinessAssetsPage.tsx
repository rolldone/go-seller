...
import { memberDelete, memberGet, memberPost, memberPostForm, memberPut } from "../businesses/api";
import MemberAssetBundleField, {
  type ExistingMemberAsset,
  type ExistingMemberAssetFolder,
  type MemberAssetRulesConfig,
  validateMemberFilesAgainstUsageConfig,
} from "../ui/MemberAssetBundleField";
...
type AssetListResponse = {
  data: ExistingMemberAsset[];
};

type FolderListResponse = {
  data: ExistingMemberAssetFolder[];
};

const businessAssetUsageConfig: MemberAssetRulesConfig = {
  type: "business_asset",
  rules: {
    cover: {
      label: "Cover",
...
export default function BusinessAssetsPage() {
  ...
  const [folders, setFolders] = useState<ExistingMemberAssetFolder[]>([]);
  const [selectedFolderID, setSelectedFolderID] = useState("");
  const [assets, setAssets] = useState<ExistingMemberAsset[]>([]);
  ...
  const loadBusinesses = async () => {
    ...
    try {
      ...
      setBusinesses(list);
      const queryBusinessID = typeof window !== "undefined" ? new URLSearchParams(window.location.search).get("business_id")?.trim() || "" : "";
  ...
  const loadFolders = async (businessID: string) => {
    const res = await memberGet<FolderListResponse>(`/api/member/businesses/${businessID}/asset-folders`);
  ...
  const loadAssets = async (businessID: string, folderID: string) => {
    ...
    try {
      ...
      const res = await memberGet<AssetListResponse>(`/api/member/businesses/${businessID}/assets?limit=200${folderQuery}`);
  ...
  const handleCreateFolder = async (name: string, parentID?: string) => {
    if (!selectedBusinessID) return;
    try {
      await memberPost(`/api/member/businesses/${selectedBusinessID}/asset-folders`, {
        name,
  ...
  const handleRenameFolder = async (folder: ExistingMemberAssetFolder, nextName: string) => {
    if (!selectedBusinessID) return;
    try {
      await memberPut(`/api/member/businesses/${selectedBusinessID}/asset-folders/${folder.id}`, { name: nextName });
      await loadFolders(selectedBusinessID);
  ...
  const handleDeleteFolder = async (folder: ExistingMemberAssetFolder) => {
    if (!selectedBusinessID) return;
    if (typeof window !== "undefined" && !window.confirm(`Delete folder "${folder.path}"?`)) {
      return;
    }
    try {
      await memberDelete(`/api/member/businesses/${selectedBusinessID}/asset-folders/${folder.id}`);
      setSelectedFolderID("");
  ...
  const handleUpload = async () => {
    ...
    const validation = await validateMemberFilesAgainstUsageConfig(selectedFiles, usageTag, businessAssetUsageConfig);
    if (validation) {
    ...
    try {
      for (const [index, uploadFile] of selectedFiles.entries()) {
        ...
        await memberPostForm(`/api/member/businesses/${selectedBusinessID}/assets/upload`, formData);
      }
  ...
  const handleSetMain = async (asset: ExistingMemberAsset) => {
    if (!selectedBusinessID) return;
    try {
      await memberPut(`/api/member/businesses/${selectedBusinessID}/assets/${asset.id}`, {
        is_main: true,
  ...
  const handleDeleteAsset = async (asset: ExistingMemberAsset) => {
    if (!selectedBusinessID) return;
    if (typeof window !== "undefined" && !window.confirm(`Delete asset "${asset.original_name || asset.file_path}"?`)) {
      return;
    }
    try {
      await memberDelete(`/api/member/businesses/${selectedBusinessID}/assets/${asset.id}`);
      await loadAssets(selectedBusinessID, selectedFolderID);
  ...
  const handleUpdateAsset = async (
    asset: ExistingMemberAsset,
    patch: { usage_tag?: string; display_order?: number; folder_id?: string | null },
  ) => {
    if (!selectedBusinessID) return;
    try {
      await memberPut(`/api/member/businesses/${selectedBusinessID}/assets/${asset.id}`, patch);
      await loadAssets(selectedBusinessID, selectedFolderID);
  ...
  const handleCopyLink = async (asset: ExistingMemberAsset) => {
    try {
  ...
  const handleDuplicateAsset = async (asset: ExistingMemberAsset) => {
    if (!selectedBusinessID) return;
    try {
      await memberPost(`/api/member/businesses/${selectedBusinessID}/assets/${asset.id}/copy`, {
    ...
    <div className="space-y-6">
      <div className="rounded-xl border border-sky-200 bg-sky-50 p-4 text-sm text-sky-900">
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          <a href={buildLocalizedPath("/member/businesses", locale)} className="rounded-lg border border-sky-200 bg-white px-3 py-1.5 text-xs font-semibold text-sky-700 hover:bg-sky-100">
