...
import { createMemberDiscount, deleteMemberDiscount, listMemberDiscounts, updateMemberDiscount } from "./api";
import DiscountFormModal, { discountToForm, emptyForm, formToPayload } from "./DiscountFormModal";
...
export default function ProductDiscountsModal({ open, product, onClose }: Props) {
	...
	const loadDiscounts = useCallback(async () => {
		...
		setError(null);
		try {
			const res = await listMemberDiscounts(product.id, { q: "", is_active: "", page: 1, limit: defaultLimit });
	...
	const handleEdit = (discount: Discount) => {
		...
		setFormState({ ...discountToForm(discount), product_ids: product?.id ?? discount.product_ids?.[0] ?? "", selectedProductIDs: product ? [product.id] : discount.product_ids ?? [] });
		setFormOpen(true);
	};
	...
	const handleFormSubmit = async () => {
		...
		try {
			...
			if (formMode === "create") {
				await createMemberDiscount(product.id, payload);
				notifySuccess("Discount created");
			} else if (selectedDiscount) {
				await updateMemberDiscount(product.id, selectedDiscount.id, payload);
	...
	const handleConfirmDelete = async () => {
		if (!product || !selectedDiscount) return;
		setDeleteSubmitting(true);
		try {
			await deleteMemberDiscount(product.id, selectedDiscount.id);
			...
			<div className="flex h-full w-full max-w-5xl flex-col overflow-hidden rounded-2xl bg-white">
				<div className="flex items-center justify-between border-b border-slate-100 p-5">
					...
					<div className="flex items-center gap-2 text-sm text-slate-500">
						...
						<button type="button" onClick={onClose} className="rounded bg-slate-100 px-3 py-1 text-xs text-slate-700 hover:bg-slate-200">
							Close
						</button>
					</div>
				</div>
				...
				<div className="flex items-center justify-between border-b border-slate-100 p-5">
					<button type="button" onClick={handleOpenForm} className="rounded bg-emerald-600 px-3 py-2 text-sm font-medium text-white hover:bg-emerald-500">
						+ New Discount
					</button>
