...
export default function BusinessTranslationsModal({ open, business, onClose }: Props) {
	...
	const applyTranslation = (translation: BusinessTranslation | null) => {
		if (!translation) {
			importFromBusiness();
			return;
		}
		...
		setForm({
			...
			highlights: Array.isArray(translation.highlights) ? translation.highlights.filter(Boolean).join("\n") : "",
		});
		setStoryValue({
			html: translation.story_html || translation.story_plain || "",
			plain: translation.story_plain || translation.story_html || "",
			blocks: (translation.story_blocks as JSONContent) || { type: "doc", content: [] },
		});
	};
	...
	const refresh = async (activeLocale = locale) => {
		...
		setLoading(true);
		try {
			const res = await memberGet<{ data: BusinessTranslation[] }>(`/api/member/businesses/${business.id}/translations`);
	...
	useEffect(() => {
		...
		importFromBusiness();
	}, [locale, open, business, currentTranslation]);
	...
	const handleSave = async () => {
		...
		try {
			...
			await memberPut(`/api/member/businesses/${business.id}/translations/${locale}`, {
				...
				<div className="space-y-5">
					...
					<div className="grid gap-2 sm:grid-cols-2">
						...
						<div className="rounded-2xl border border-slate-200 bg-slate-50 px-3.5 py-2.5 text-xs text-slate-600 flex items-center justify-between gap-3">
							...
							<button type="button" onClick={() => importFromBusiness()} className="rounded-lg border border-slate-200 bg-white px-3 py-1 text-xs text-slate-700 hover:bg-slate-100">
					...
					<MemberRichTextEditor value={storyValue.html} placeholder="Tulis story bisnis" onChange={setStoryValue} />
				</div>
			) : null}
		</MemberModal>
	);
