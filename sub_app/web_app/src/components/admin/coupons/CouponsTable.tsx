...
const formatCategory = (c?: string) => {
  switch (c) {
    case "product_discount":
      return "Product Discount (per product)";
    case "total_discount":
      return "Total Discount (order subtotal)";
...
export default function CouponsTable({ coupons, loading, error, onEdit, onDelete }: Props) {
  if (loading) return <div className="text-sm text-slate-500">Loading coupons...</div>;
  if (error) return <div className="text-sm text-red-600">Error: {error}</div>;
  if (coupons.length === 0) return <div className="text-sm text-slate-500">Belum ada coupon.</div>;
        ...
        <tbody>
          {coupons.map((d) => (
            <tr key={d.id} className="border-t border-slate-100">
              ...
              <td className="px-3 py-2 font-mono text-xs text-slate-700">{d.code}</td>
              ...
              <td className="px-3 py-2 text-slate-700 text-xs">
                {d.product_ids && d.product_ids.length > 0 ? (
                  <>
                    <div>{d.product_ids.length} product{d.product_ids.length > 1 ? "s" : ""}</div>
                    <div className="text-slate-400">{shortID(d.product_ids[0])}</div>
                  </>
                ) : (
                  <div className="text-slate-400">All products</div>
                )}
              </td>
              <td className="px-3 py-2">
