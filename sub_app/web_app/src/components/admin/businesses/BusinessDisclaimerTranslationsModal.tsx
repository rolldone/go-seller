...
export default function BusinessDisclaimerTranslationsModal({ open, businessID, disclaimer, onClose }: Props) {
  ...
  const refresh = async (activeLocale = locale) => {
    ...
    setLoading(true);
    try {
      const res = await adminGet<{ data: BusinessDisclaimerTranslation[] }>(
        `/admin/catalog/businesses/${businessID}/disclaimers/${disclaimer.id}/translations`,
      );
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await adminPut(`/admin/catalog/businesses/${businessID}/disclaimers/${disclaimer.id}/translations/${locale}`, payload);
        ...
        <div className="space-y-5">
          ...
          <div className="grid gap-2 sm:grid-cols-2">
            ...
            <div className="rounded-2xl border border-slate-200 bg-slate-50 px-3.5 py-2.5 text-xs text-slate-600 flex items-center justify-between gap-3">
              ...
              <button
                type="button"
                onClick={() => importFromDisclaimer(disclaimer)}
          ...
          <label className="text-sm block">
            ...
            <input
              ...
              placeholder="Localized disclaimer title"
            />
          </label>

          <RichTextEditor
            value={bodyValue.html}
            title="Deskripsi"
            helperText="Terjemahkan isi disclaimer yang tampil di popup detail."
            placeholder="Localized disclaimer content"
