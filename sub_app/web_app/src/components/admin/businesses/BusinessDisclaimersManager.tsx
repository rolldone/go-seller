...
export default function BusinessDisclaimersManager({ businessID }: Props) {
  ...
  const loadDisclaimers = async (currentBusinessID: string) => {
    ...
    try {
      const res = await adminGet<{ data: BusinessDisclaimer[] }>(`/admin/catalog/businesses/${currentBusinessID}/disclaimers?limit=100`);
  ...
  const saveDisclaimer = async () => {
    ...
    try {
      if (editingDisclaimerID) {
        await adminPut(`/admin/catalog/businesses/${businessID}/disclaimers/${editingDisclaimerID}`, payload);
        notifySuccess("Disclaimer updated");
      } else {
        await adminPost(`/admin/catalog/businesses/${businessID}/disclaimers`, payload);
  ...
  const removeDisclaimer = async (disclaimerID: string) => {
    ...
    try {
      await adminDelete(`/admin/catalog/businesses/${businessID}/disclaimers/${disclaimerID}`);
  ...
  return (
    <>
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0 flex-1">
                      <p className="text-sm font-semibold text-slate-900">
                        {disclaimer.title || "(Tanpa judul)"}
                      </p>
                      <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">
                        Order {disclaimer.sort_order || 0} · {disclaimer.is_active ? "Active" : "Inactive"}
                      </p>
                      <p className="mt-2 text-sm leading-relaxed text-slate-700 line-clamp-2 whitespace-pre-line">
                        {disclaimer.content_plain || "-"}
                      </p>
                    </div>
                    <div className="flex shrink-0 flex-col gap-2">
                      <button
                        type="button"
                        onClick={() => openEditDisclaimerModal(disclaimer)}
                      ...
                      <button
                        type="button"
                        onClick={() => openTranslationModal(disclaimer)}
                      ...
                      <button
                        type="button"
                        onClick={() => void removeDisclaimer(disclaimer.id)}
      ...
      >
        <div className="space-y-4">
          <div className="rounded-2xl border border-emerald-100 bg-emerald-50 px-4 py-3 text-sm text-emerald-900">
            Isi title terlebih dahulu, lalu tambahkan deskripsi disclaimer di bawahnya.
          </div>

          <label className="text-sm block">
            <span className="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-slate-500">Title</span>
            <input
              className="w-full rounded-xl border border-slate-200 bg-white px-3.5 py-2.5 text-sm text-slate-800 shadow-sm transition focus:border-emerald-400 focus:outline-none focus:ring-2 focus:ring-emerald-100"
              value={disclaimerForm.title}
              onChange={(e) => setDisclaimerForm((prev) => ({ ...prev, title: e.target.value }))}
              placeholder="Tulis title disclaimer"
            />
          </label>
          ...
          <label className="text-sm block">
            ...
            <select
              className="w-full rounded-xl border border-slate-200 bg-white px-3.5 py-2.5 text-sm text-slate-800 shadow-sm transition focus:border-emerald-400 focus:outline-none focus:ring-2 focus:ring-emerald-100"
              value={disclaimerForm.iconKey}
              onChange={(e) => setDisclaimerForm((prev) => ({ ...prev, iconKey: e.target.value }))}
            >
          ...
          <label className="text-sm block">
            ...
            <input
              type="number"
              className="w-full rounded-xl border border-slate-200 bg-white px-3.5 py-2.5 text-sm text-slate-800 shadow-sm transition focus:border-emerald-400 focus:outline-none focus:ring-2 focus:ring-emerald-100"
              value={disclaimerForm.sortOrder}
              onChange={(e) => setDisclaimerForm((prev) => ({ ...prev, sortOrder: e.target.value }))}
            />
          </label>
          ...
          <label className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-700">
            <input
              type="checkbox"
              checked={disclaimerForm.isActive}
              onChange={(e) => setDisclaimerForm((prev) => ({ ...prev, isActive: e.target.checked }))}
            />
            Active
          </label>
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-3">
            <RichTextEditor
              value={disclaimerForm.description.html}
              title="Deskripsi"
              helperText="Tulis deskripsi disclaimer yang tampil di popup detail."
              placeholder="Tulis isi deskripsi disclaimer"
              onChange={(value) => setDisclaimerForm((prev) => ({ ...prev, description: value }))}
            />
          </div>
      ...
      <BusinessDisclaimerTranslationsModal
        open={translationModalOpen}
        businessID={businessID}
        disclaimer={translationDisclaimer}
