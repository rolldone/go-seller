            ...
            <tr key={item.id} className="hover:bg-slate-50">
              ...
              <td className="px-4 py-3 text-sm">
                {item.fulfillment_type?.toLowerCase() === "delivery" ? (() => {
                  const delivery = deliveryBadgeMeta(t, item.delivery_status || "pending");
                  const summaries = shipmentSummary(t, item.shipments);
                  return (
                    <div className="flex flex-col gap-1">
                      <a
                        href={`/admin/orders/${item.id}/delivery`}
                        className={`inline-flex w-fit rounded-full border px-2 py-0.5 text-xs font-medium hover:opacity-80 ${delivery.className}`}
                        onClick={(e) => e.stopPropagation()}
                        title={item.delivery_status || "pending"}
                      >
                        {delivery.label}
                      </a>
                      {summaries.length > 0 ? (
