...
export default function CartPage({ customerSession = null, business = null }: CartPageProps) {
  ...
  const loadCart = async (targetBusinessID?: string | null) => {
    ...
    try {
      ...
      const message = loadError instanceof Error ? loadError.message : "Gagal memuat cart";
      setError(message);
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const message = addError instanceof Error ? addError.message : "Gagal menambahkan item ke cart";
        setError(message);
      }
    })();
    // eslint-disable-next-line react-hooks/exhaustive-deps
      ...
      if (typeof window !== "undefined") {
        if (orderID) {
          window.sessionStorage.setItem(
            ...
            "Terima kasih, pesanan Anda sudah kami terima. Tim kami akan menghubungi Anda via WhatsApp untuk konfirmasi ongkir dan total pembayaran.",
          );
        }
        window.location.href = orderID ? `/order/${encodeURIComponent(orderID)}` : "/order/confirmed";
      }
    } catch (checkoutError) {
      const message = checkoutError instanceof Error ? checkoutError.message : "Gagal checkout cart";
      setError(message);
    } finally {
  ...
  const displayMerchant = items[0]?.business_name || cart?.business_id || "Merchant";
  const checkoutLabel = productID ? (currentSearchParams?.get("intent") === "buy_now" ? "Beli Sekarang" : "Lanjut Checkout") : "Lanjut Checkout";
    ...
    <div className="min-h-screen bg-[#f7f7f5] text-slate-900">
      <main className="mx-auto max-w-7xl px-4 pb-16 pt-6 sm:px-6 lg:px-8">
        ...
        <section className="mt-8 flex items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-slate-900 sm:text-3xl">Keranjang Belanja</h1>
            <p className="mt-2 text-sm text-slate-500">Periksa item kamu sebelum lanjut ke checkout.</p>
          </div>
          <a
        ...
        {loading ? (
          ...
          <section className="mt-8 rounded-3xl border border-slate-200 bg-white p-10 text-center shadow-sm">
            ...
            <h2 className="mt-4 text-xl font-bold text-slate-900">Keranjang kamu masih kosong</h2>
            <p className="mt-2 text-sm text-slate-500">Yuk jelajahi produk terbaik dari merchant favoritmu.</p>
            <a
              href={businessHomeHref}
          ...
          <div className="mt-8 grid gap-6 lg:grid-cols-[1fr_360px]">
                ...
                <article key={item.id} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
                  <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
                    <div className="flex h-24 w-full items-center justify-center overflow-hidden rounded-xl bg-slate-100 sm:w-36">
                      {item.image_url ? (
                        <img src={item.image_url} alt={item.product_name || "Produk"} className="h-full w-full object-cover" loading="lazy" />
                      ) : (
                    ...
                    <div className="min-w-0 flex-1">
                      <h3 className="truncate text-base font-semibold text-slate-900">{item.product_name || "Produk"}</h3>
                      <p className="mt-1 inline-flex items-center gap-1 text-xs text-slate-500">
                        <Store className="h-3.5 w-3.5" />
                        {item.business_name || displayMerchant}
                      </p>
                      <p className="mt-1 text-xs text-slate-500">
                        Variasi: {item.sku || item.variation_id || "Produk utama"}
                      </p>
                      {previewItem ? (
                        <div className="mt-2 flex flex-wrap gap-2 text-[11px] font-semibold">
                          ...
                          <span className="rounded-full bg-emerald-50 px-2 py-0.5 text-emerald-700">
                            Pajak {toCurrency(previewItem.tax_amount)}
                          </span>
                        </div>
                      ) : null}
                      <p className="mt-3 text-sm font-bold text-slate-900">{toCurrency(item.unit_price)}</p>
                      <p className="mt-1 text-xs text-slate-500">Total item: {toCurrency(item.total_price || item.unit_price * item.qty)}</p>
                    </div>
            ...
            <aside className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm lg:sticky lg:top-6 lg:h-fit">
              <h2 className="text-lg font-bold text-slate-900">Ringkasan Belanja</h2>

              <div className="mt-2 text-xs uppercase tracking-[0.18em] text-slate-500">{displayMerchant}</div>

              <div className="mt-4 space-y-3 text-sm text-slate-600">
                <div className="space-y-2 rounded-xl border border-slate-200 bg-slate-50 p-3">
                  <div className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">Alamat Pengiriman</div>
                  {addresses.length === 0 ? (
                    <div className="text-xs text-amber-700">
                      Belum ada alamat tersimpan.{" "}
                      <a href="/customer/dashboard?tab=addresses" className="font-medium underline hover:text-amber-900">
                        Tambahkan alamat di dashboard
                      </a>{" "}
                      sebelum checkout.
                    </div>
                  ) : (
                      ...
                      {taxGroups.map((group) => (
                        <div key={`${group.taxType}-${group.taxRate}`} className="flex items-center justify-between gap-3">
                          <span className="capitalize text-slate-500">
                            Pajak {formatTaxMode(group.taxType, group.taxRate)}
                          </span>
              ...
              <div className="mt-4">
                <label className="mb-2 block text-xs font-semibold text-slate-600">Kode Kupon</label>
                <div className="flex gap-2">
                  ...
                  >
                    {applyingCoupon ? "Memeriksa..." : "Pasang"}
                  </button>
                </div>
                {preview?.applied_coupons && preview.applied_coupons.length > 0 ? (
                  <div className="mt-2 text-xs text-slate-600">Kupon aktif: {preview.applied_coupons.map((c) => c.code).join(", ")}</div>
                ) : null}
              </div>
              ...
              >
                {submittingCheckout ? "Memproses checkout..." : checkoutLabel}
              </button>
            </aside>
          </div>
        )}
