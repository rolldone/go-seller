...
export default function ResetPasswordPage({ locale }: ResetPasswordPageProps) {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      window.location.href = buildLocalizedPath("/customer/auth/login", locale);
    } catch (submitError) {
    ...
    <div className="flex min-h-screen flex-col items-center justify-center bg-[#f7f7f5] px-4">
      <div className="w-full max-w-sm">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white px-8 py-10 shadow-sm">
          <h1 className="mb-1 text-2xl font-bold text-slate-900">{t("resetTitle", "Buat Password Baru")}</h1>
          <p className="mb-8 text-sm text-slate-500">{t("resetSubtitle", "Password baru harus berbeda dari password yang sebelumnya.")}</p>
          ...
          <form className="space-y-5" onSubmit={handleSubmit}>
            ...
            <div>
              <label className="mb-1.5 block text-sm font-medium text-slate-700">
                {t("newPasswordConfirm", "Konfirmasi Password Baru")}
              </label>
              <input
                type="password"
                autoComplete="new-password"
            ...
            >
              {loading ? "Memproses..." : t("resetPassword", "Reset Password")}
            </button>
          </form>
