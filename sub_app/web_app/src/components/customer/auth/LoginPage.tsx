  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        ...
        if (cancelled) return;
        const target = consumeCustomerAuthNextPath(window.location.search) || buildLocalizedPath("/customer/dashboard", locale);
    ...
    <div className="flex min-h-screen flex-col items-center justify-center bg-[#f7f7f5] px-4">
      <div className="w-full max-w-sm">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white px-8 py-10 shadow-sm">
          <h1 className="mb-1 text-2xl font-bold text-slate-900">{t("loginTitle", "Masuk")}</h1>
          <p className="mb-8 text-sm text-slate-500">{t("loginSubtitle", "Selamat datang kembali di GoSeller")}</p>
          ...
          >
            ...
            <div>
              <div className="mb-1.5 flex items-center justify-between">
                <label className="text-sm font-medium text-slate-700">{t("password", "Password")}</label>
                <a href={buildLocalizedPath("/customer/auth/forgot-password", locale)} className="text-xs text-emerald-600 hover:underline">
            ...
            >
              {loading ? "Memproses..." : t("loginButton", "Masuk")}
            </button>
          </form>
