...
type CustomerVerifyEmailPageProps = {
	locale?: string;
};
...
export default function CustomerVerifyEmailPage({ locale }: CustomerVerifyEmailPageProps) {
	...
	useEffect(() => {
		...
		async function verifyEmail() {
			try {
				...
				const response = await fetch(`${apiBase}/api/customer/auth/verify?token=${encodeURIComponent(token)}`, {
					method: "GET",
			...
			<div className="mx-auto flex w-full max-w-lg flex-col items-center justify-center py-10">
				<div className="w-full rounded-3xl border border-slate-200 bg-white/95 p-6 shadow-[0_20px_80px_rgba(15,23,42,0.08)] backdrop-blur sm:p-8">
					<div className="flex flex-col items-center text-center">
						...
						<p className="text-xs font-semibold uppercase tracking-[0.2em] text-emerald-600">Verifikasi Email Customer</p>
